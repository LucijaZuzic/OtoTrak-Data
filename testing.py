...
def plot_isomap(manifold_2D, longs_matrix, lats_matrix, save_name):  
    ...
    plt.plot(longs_matrix[top_left_index], lats_matrix[top_left_index], color = 'blue')
    plt.subplot(3, 4, 3)
    ...
    plt.plot(longs_matrix[top_center_index], lats_matrix[top_center_index], color = 'orange') 
    plt.subplot(3, 4, 4)
    plt.title("Top right")
    ...
    plt.plot(longs_matrix[top_right_index], lats_matrix[top_right_index], color = 'green') 
    ...
    plt.plot(longs_matrix[mid_left_index], lats_matrix[mid_left_index], color = 'red')
    plt.subplot(3, 4, 7)
    ...
    plt.plot(longs_matrix[mid_center_index], lats_matrix[mid_center_index], color = 'purple') 
    plt.subplot(3, 4, 8)
    plt.title("Mid right")
    ...
    plt.plot(longs_matrix[mid_right_index], lats_matrix[mid_right_index], color = 'brown') 
    ...
    plt.plot(longs_matrix[bottom_left_index], lats_matrix[bottom_left_index], color = 'pink')
    plt.subplot(3, 4, 11)
    ...
    plt.plot(longs_matrix[bottom_center_index], lats_matrix[bottom_center_index], color = 'gray') 
    plt.subplot(3, 4, 12)
    ...
    plt.plot(longs_matrix[bottom_right_index], lats_matrix[bottom_right_index], color = 'olive') 
...
def make_cluster(manifold_2D):
    ...
    clustering_DBSCAN = DBSCAN(eps = 2000, min_samples = 2).fit(X_clus)
...
use_multi_isomap = True
draw_multi_isomap = True

use_multi_DBSCAN = True
draw_multi_DBSCAN = True
    ...
    for some_file in all_files:  
        ...
        for x in range(0, len(longitudes) - window_size + 1, step_size):
            ...
            if len(set_lats) == 1 and len(set_longs) == 1: 
                ...
                skipped_frames += 1
                continue  
            longitudes_tmp2, latitudes_tmp2, quadrant = preprocess_long_lat(longitudes_tmp, latitudes_tmp) 
            quadrants[quadrant] += 1
            if quadrant == 3:
                plt.subplot(1, 2, 1)
                plt.plot(longitudes_tmp2, latitudes_tmp2)
                plt.subplot(1, 2, 2)
                plt.plot(longitudes_tmp, latitudes_tmp)
                plt.plot(longitudes_tmp[0], latitudes_tmp[0], marker = '.', linewidth = 10, color = 'r')
                plt.show()
    print(quadrants)